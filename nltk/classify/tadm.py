...
def parse_tadm_weights(paramfile: Iterable[str]) -> numpy.ndarray:
    """Parses the tadm weights from the given file.

    Parameters
    ----------
    paramfile : Iterable[str]
        An iterable of strings representing lines from a parameter file.
    Returns
    -------
    numpy.ndarray
        A NumPy array containing the weight vector.
    """
    return numpy.fromiter((float(line.strip()) for line in paramfile), dtype="d")
...
def parse_large_file_with_memory_map(paramfile_path: str) -> numpy.ndarray:
    """Parses the tadm weights using memory-mapped files for extremely large files.
    ...
    numpy.ndarray
        A NumPy array containing the weight vector.
    """
    with open(paramfile_path, "r+b") as f:
        mmapped_file = mmap.mmap(f.fileno(), 0, access=mmap.ACCESS_READ)
        try:
            return numpy.fromiter(
                (
                    float(line.decode().strip())
